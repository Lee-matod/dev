        ...
        try:
            ...
            if context.command.name == "python":
                ...
                if python_class.error:
                    error = python_class.error[0]
                    await send(ctx, is_py_bt=True, embed=discord.Embed(title=error.__class__.__name__, description="".join(format_exception(error, error, error.__traceback__)), color=discord.Color.red()))

        except Exception as e:
            err = "".join(format_exception(e, e, e.__traceback__))
            if settings["folder"]["path_to_file"]:
                err.replace(settings["folder"]["path_to_file"], "/path/to/file/")
