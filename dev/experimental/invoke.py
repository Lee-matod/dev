...
class RootInvoke(Root):
    ...
    async def root_debug(self, ctx: commands.Context, *, command_args):
        ...
        if handler.error:
            exc_val, tb = handler.error
            await send(ctx, is_py_bt=True, embed=discord.Embed(title=exc_val.__class__.__name__, description=tb.replace(settings["path_to_file"], ""), color=discord.Color.red()))
