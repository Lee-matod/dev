...
class RootHTTP(commands.Cog):
    ...
    @root.group(name="http", parent="dev", version=1.1, invoke_without_command=True)
    @is_owner()
    async def root_http(self, ctx: commands.Context):
        pass

    @root_http.command(name="get", version=1.1, supports_virtual_vars=True)
    @is_owner()
    async def root_http_get(self, ctx: commands.Context, url: str, mode: str, allow_redirects: bool = False):
