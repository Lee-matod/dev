...
class Dropdown(discord.ui.View):
    ...
    def __init__(self, ctx: commands.Context, embed: discord.Embed, *, cogs: Ls, cmds: Ls, channels: Ls, emojis: Ls, members: Ls, roles: Ls):
        self.mapping = {
...
class RootSearch(Root):
    ...
    async def root_search(self, ctx: commands.Context, *, query: str):
        ...
        cmds = match(query, [(cmd.qualified_name, f"`{cmd.qualified_name}`") for cmd in all_commands(self.bot.commands)])
        ...
        embed = discord.Embed(title=f"Query {query} returned...", description=join_multi_iter([cogs, cmds, channels, emojis, members, roles], max_amount=7), color=discord.Color.blurple())
        embed.set_footer(text="Category: All")
        view = Dropdown(ctx, embed, cogs=cogs, cmds=cmds, channels=channels, emojis=emojis, members=members, roles=roles)
