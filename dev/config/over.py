...
class RootOverrideBot(commands.Cog):
    ...
    @root.group(name="override", parent="dev", version=1.1)
    @is_owner()
    ...
    async def root_override_command(self, ctx: commands.Context, *, command_code: StringCodeblockConverter):
        """Temporarily override a command. All changes will be undone once the bot is restart or the cog is reloaded. This differentiates from its counterpart `dev overwrite` which permanently changes a file.
        ...
        try:
            ...
            return await ctx.send("Malformed arguments have been given.")
    ...
    async def root_overwrite_command(self, ctx: commands.Context, *, command_code: StringCodeblockConverter):
        ...
        if not command:
            return await ctx.send(f"Command `{command_string}` is not found.")
