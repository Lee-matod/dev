...
from dev.converters import StringCodeblockConverter
...
class RootOverride(Root):
    ...
    @root_override.command(name="command", version=1, supports_virtual_vars=True, usage="<command_name> <script>")
    async def root_override_command(self, ctx: commands.Context, *, command_code: StringCodeblockConverter):
        ...
        COMMAND_OVERRIDES: Dict[str, Callable] = getattr(RootOverride, "COMMAND_OVERRIDES", None)
        ...
        with VirtualVarReplacer(settings, script) as decoded_script:
            script = clean_code(decoded_script)
        local_vars = {
            "discord": discord,
            "commands": commands,
            "bot": self.bot,
    ...
    @root_override.command(name="setting", version=1, supports_virtual_vars=True, aliases=["settings"], usage="<setting>... <command_name|script>")
    async def root_override_setting(self, ctx: commands.Context, *, greedy: OverrideSettingConverter):
        ...
        """
        if greedy.script:
            with VirtualVarReplacer(settings, greedy.script) as decoded_script:
                code = clean_code(decoded_script)
            local_vars = {
                "discord": discord,
                "commands": commands,
                "bot": self.bot,
    ...
    @root_overwrite.command(name="command", version=1, supports_virtual_vars=True, usage=r"<command_name> <script>")
    async def root_overwrite_command(self, ctx: commands.Context, *, command_code: StringCodeblockConverter):
        ...
        # `self` is passed as :type commands.Context: hence why we use `getattr` to get our class attributes.
        COMMAND_OVERRIDES: Dict[str, Callable] = getattr(RootOverride, "COMMAND_OVERRIDES", None)
        COMMAND_OVERWRITES: Dict[str, Callable] = getattr(RootOverride, "COMMAND_OVERWRITES", None)  # this has no use yet, but will probably serve a purpose in order to undo any unwanted overwrites
        command_string, script = command_code
        if not command_string or not script:
